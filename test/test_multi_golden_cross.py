"""
单元测试：多金叉共振策略
测试目标：验证多金叉共振策略的所有功能，包括指标计算、金叉识别、共振确认等
"""
import unittest
import pandas as pd
import numpy as np
...
class TestMultiGoldenCrossIndicators(unittest.TestCase):
    """测试指标计算功能"""
    ...
    def setUp(self):
        ...
        self.strategy = MultiGoldenCrossStrategy()
...
class TestMultiGoldenCrossGoldenCross(unittest.TestCase):
    """测试金叉识别功能"""
    ...
    def test_find_ma_cross_returns_date_or_none(self):
        """测试_find_ma_cross返回日期或None"""
        # 计算指标
        df_with_indicators = self.strategy.calculate_indicators(self.test_df)
        
        # 查找均线金叉
        ma_cross_date = self.strategy._find_ma_cross(df_with_indicators.head(10))
        
        # 验证返回类型
        self.assertTrue(ma_cross_date is None or isinstance(ma_cross_date, pd.Timestamp))
    
    def test_find_kdj_cross_returns_date_or_none(self):
        """测试_find_kdj_cross返回日期或None"""
        # 计算指标
        df_with_indicators = self.strategy.calculate_indicators(self.test_df)
        
        # 查找KDJ金叉
        kdj_cross_date = self.strategy._find_kdj_cross(df_with_indicators.head(10))
        
        # 验证返回类型
        self.assertTrue(kdj_cross_date is None or isinstance(kdj_cross_date, pd.Timestamp))
    
    def test_find_macd_cross_returns_date_or_none(self):
        """测试_find_macd_cross返回日期或None"""
        # 计算指标
        df_with_indicators = self.strategy.calculate_indicators(self.test_df)
        
        # 查找MACD金叉
        macd_cross_date = self.strategy._find_macd_cross(df_with_indicators.head(10))
        
        # 验证返回类型
        self.assertTrue(macd_cross_date is None or isinstance(macd_cross_date, pd.Timestamp))


class TestMultiGoldenCrossResonance(unittest.TestCase):
    """测试共振确认功能"""
...
class TestMultiGoldenCrossPriceConfirmation(unittest.TestCase):
    """测试价格确认功能"""
    ...
    def test_check_price_confirmation_with_price_below_ma(self):
        """测试_check_price_confirmation价格低于均线的情况"""
        # 创建价格低于均线的最新数据
        latest = pd.Series({
            'close': 90.0,
            'ma_short': 100.0,
...
class TestMultiGoldenCrossSelectStocks(unittest.TestCase):
    """测试选股功能"""
...
class TestMultiGoldenCrossIntegration(unittest.TestCase):
    """测试集成功能"""
